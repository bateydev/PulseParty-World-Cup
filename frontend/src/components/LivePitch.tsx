...
export function LivePitch() {
  ...
  useEffect(() => {
    ...
    setPlayers(prev => prev.map(player => {
      const distanceToBall = Math.sqrt(Math.pow(player.x - newX, 2) + Math.pow(player.y - newY, 2));
    ...
    <div className="live-pitch w-full">
      ...
      <div className="flex items-center justify-center gap-6 mb-4">
        <div className="text-center">
          <div className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-1">HOME</div>
          <div className="text-4xl font-bold text-blue-600 dark:text-blue-400">{currentScore.home}</div>
        </div>
        <div className="text-3xl font-bold text-gray-400">-</div>
        <div className="text-center">
          <div className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-1">AWAY</div>
          <div className="text-4xl font-bold text-red-600 dark:text-red-400">{currentScore.away}</div>
        </div>
      </div>
      ...
      <div className="relative w-full aspect-[16/10] bg-gradient-to-b from-green-600 to-green-700 rounded-2xl shadow-2xl overflow-hidden border-4 border-white dark:border-gray-700">
        ...
        {possession && (
          <div className={`absolute top-4 ${possession === 'home' ? 'left-4' : 'right-4'} px-3 py-1 rounded-full text-xs font-bold text-white shadow-lg animate-pulse ${
